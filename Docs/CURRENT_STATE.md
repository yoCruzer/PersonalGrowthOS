...
| Governance status | Build 8 real V7 migration and additive Local Day metadata are locally verified; the branch is not ready for review or distribution |
...
| Final automated gate | Build 8 Stage 1 targeted migration/Habit/transfer: 48/48 PASS; the complete Build 8 gate has not run |
| Release gate | Build 7 physical-device validation remains external; do not merge, archive or upload TestFlight automatically |
| Next checkpoint | Make `recordingMode` an explicit historical Plan fact and preserve it through validation, migration and backup v4 |

## Build 8 In-progress Foundation

Build 8 starts at exact SHA `95076bf5c2a86122fbd327903d80bccdfb8c768d` on a dedicated stacked branch. Remote head `bdb17449af6db016b70fd404712b2342e27e8504` contains the first-pass implementation. A fixture generated by the exact Build 7 source proved that adding Local Day fields directly to the shared `HabitLog` changed the V7 model checksum and made the real store an unknown staged-migration version. Stage 1 restores the exact V7 `HabitLog` shape and stores V8 civil-day, time-zone and provenance facts in a purely additive `HabitLogDayMetadata` entity. The real V7 store now migrates, bootstraps idempotently, preserves representative IDs/relationships/media and reopens unchanged across a time-zone change. The implementation remains incomplete and has no review handoff yet. See `Docs/BUILD8_HABIT_ANALYTICS.md` for its verified scope, current work and known gaps.
