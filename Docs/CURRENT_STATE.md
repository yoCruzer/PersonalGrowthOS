...
| Governance status | Build 8 migration, historical Plan semantics and effective-Plan runtime truth are locally verified; the branch is not ready for review or distribution |
...
| Final automated gate | Build 8 Stage 3 Habit Foundation: 49/49 PASS; the complete Build 8 gate has not run |
| Release gate | Build 7 physical-device validation remains external; do not merge, archive or upload TestFlight automatically |
| Next checkpoint | Separate current and pending Plan editor semantics, including name-only edits |

## Build 8 In-progress Foundation

Build 8 starts at exact SHA `95076bf5c2a86122fbd327903d80bccdfb8c768d` on a dedicated stacked branch. Remote head `bdb17449af6db016b70fd404712b2342e27e8504` contains the first-pass implementation. A fixture generated by the exact Build 7 source proved that adding Local Day fields directly to the shared `HabitLog` changed the V7 model checksum and made the real store an unknown staged-migration version. Stage 1 restores the exact V7 `HabitLog` shape and stores V8 civil-day, time-zone and provenance facts in a purely additive `HabitLogDayMetadata` entity. Stage 2 makes `recordingMode` an explicit historical Plan/revision/v4-transfer fact. Stage 3 makes the Plan effective on the relevant Local Day authoritative for Today, Overview, Detail and check-in behavior; `HabitConfiguration` is now only a fallback when no effective Plan exists for legacy/bootstrap compatibility. The real V7 store still migrates and reopens with representative facts intact. The implementation remains incomplete and has no review handoff yet. See `Docs/BUILD8_HABIT_ANALYTICS.md` for its verified scope, current work and known gaps.
