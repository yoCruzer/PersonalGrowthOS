...
| Governance status | Build 8 migration, Plan correctness and contiguous analytics are locally verified; the branch is not ready for review or distribution |
...
| Final automated gate | Build 8 Stage 5 Habit Foundation: 54/54 PASS; the complete Build 8 gate has not run |
| Release gate | Build 7 physical-device validation remains external; do not merge, archive or upload TestFlight automatically |
| Next checkpoint | Finish Local Day and lifecycle historical truth, including timestamp ordering and migration neutrality |

## Build 8 In-progress Foundation

Build 8 starts at exact SHA `95076bf5c2a86122fbd327903d80bccdfb8c768d` on a dedicated stacked branch. Remote head `bdb17449af6db016b70fd404712b2342e27e8504` contains the first-pass implementation. A fixture generated by the exact Build 7 source proved that adding Local Day fields directly to the shared `HabitLog` changed the V7 model checksum and made the real store an unknown staged-migration version. Stage 1 restores the exact V7 `HabitLog` shape and stores V8 civil-day, time-zone and provenance facts in a purely additive `HabitLogDayMetadata` entity. Stages 2–4 make Plan recording mode historical, effective Plan authoritative at runtime, and pending edits explicit/non-destructive. Stage 5 restricts current adherence, consistency and streaks to the current contiguous Plan/lifecycle segment, so returning to a prior period cannot bridge across intervening semantics. The implementation remains incomplete and has no review handoff yet. See `Docs/BUILD8_HABIT_ANALYTICS.md` for its verified scope, current work and known gaps.
