...
当 Owner 明确启动本 Program 后，它将 Owner 已接受的 Foundation Documents、`DEVELOPMENT_CONTRACT.md` 和 `V1_IMPLEMENTATION_PLAN.md` 转化为一次可持续执行的 Agentic Engineering Program，使 Codex 能够在明确边界内：
...
Personal Growth OS 的默认治理模式仍然是逐子 Stage 审批：
...
只有当 Owner 发出明确 startup instruction，且 `Docs/CURRENT_TASK.md` 已记录该启动授权时，V1 才采用以下自主执行方式：
...
在 Program Authorized / Running 状态中，Macro Stage 不再是逐个等待 Owner 批准的行政审批边界。在此状态之前，默认逐子 Stage 审批继续生效。

---

# Authority and Precedence

自主执行期间，Codex 必须完全服从 `DEVELOPMENT_CONTRACT.md` 已定义的权威顺序：

1. `Docs/INDEX.md` 及其定义的 Foundation Documents。
2. `DEVELOPMENT_CONTRACT.md`。
3. `Docs/CURRENT_STATE.md`，用于记录可验证的仓库事实。
4. `Docs/CURRENT_TASK.md`，用于记录当前工作边界和 Owner 是否已批准启动 Program。
5. 已被 Owner 接受且与当前任务相关的 implementation / execution plans，其中包括本文档和 `Docs/V1_IMPLEMENTATION_PLAN.md`。
6. 现有代码、测试、构建结果和 Git 历史。

其中：

* Foundation Documents 继续决定产品方向和 V1 范围；
* `DEVELOPMENT_CONTRACT.md` 继续决定长期开发原则；
* 本文档不能重新排列或覆盖 `DEVELOPMENT_CONTRACT.md`；
* 本文档只定义 S1–S10 在获得明确 startup authorization 后的一次性自主执行方式；
* `V1_IMPLEMENTATION_PLAN.md` 继续定义 Macro Stage 的能力范围、依赖关系和 Exit Criteria；
* 本文档仅覆盖原有“每个子 Stage 必须单独获得 Owner 批准”的执行机制。

Governance Baseline 被接受不等于 Program 已启动。只有在 `Docs/CURRENT_TASK.md` 明确记录 Owner 已批准启动 V1 Autonomous Build Program 后，本文档才成为当前可执行授权。如果 `Docs/CURRENT_TASK.md` 尚未记录启动授权，Codex 必须停止在启动前边界。

如果低优先级文档与高优先级文档冲突，必须遵循高优先级文档。如果本文档与 `DEVELOPMENT_CONTRACT.md` 冲突，Contract 优先，Codex 必须停止并升级给 Owner。如果继续执行需要修改或绕过 `DEVELOPMENT_CONTRACT.md`，Codex 必须停止并升级，不得自行修改 Contract。

不得为了方便实现而静默修改、弱化或重新解释 Foundation 决策。

---

# Governance and Program State Machine

本 Program 采用以下三个互斥状态。

## State 1 — Owner Review Draft

* Governance 文档正在审查或复核；
* S1–S10 未获启动授权；
* 不得创建 `feat/v1-autonomous-build`；
* 当前 `docs/v1-autonomous-governance` 治理分支处于此状态。

## State 2 — Owner Accepted Governance Baseline — Awaiting Startup Authorization

* Governance 机制已经被 Owner 接受并进入 `main`；
* S1–S10 仍未启动；
* `feat/v1-autonomous-build` 尚未创建；
* 仍需要一次明确、独立的 Owner startup instruction。

## State 3 — Program Authorized / Running

* Owner 已给出明确 startup instruction；
* `Docs/CURRENT_TASK.md` 已记录启动授权；
* 已从准确、clean 的 `main` SHA 创建 `feat/v1-autonomous-build`；
* Codex 可以自主执行已授权的 S1–S10。

唯一合法的前进转换是：

```text
Owner Review Draft
→ Owner Accepted Governance Baseline — Awaiting Startup Authorization
→ Program Authorized / Running
```

Governance 接受不得被解释为自动进入 Program Authorized / Running。任何跳过 State 2 或缺少 `CURRENT_TASK.md` 启动记录的执行都必须停止。
...
Governance Adoption 分为三个独立步骤，不得合并或自动跨越。

## Governance Preparation

1. 创建和修订本文档及相关治理文件；
2. 更新 `Docs/V1_IMPLEMENTATION_PLAN.md` 的 Macro Stage Governance；
3. 更新 `Docs/CURRENT_STATE.md`，记录真实 Git、S0 和产品状态；
4. 将 `Docs/CURRENT_TASK.md` 设置为 Governance Review 或 Governance Re-review；
5. 创建或更新 `Docs/V1_AUTONOMOUS_STATUS.md`；
6. 不开始 Program，不创建执行分支，不修改产品代码；
7. 形成独立、可审查的 Governance Commit；
8. 提交后停止，等待 Independent Governance Reviewer 和 Owner。

Governance Preparation 始终处于 State 1，不得提前标记为 Accepted。

## Governance Finalization

仅在 Independent Governance Reviewer 给出 PASS 且 Owner 明确接受后执行：

1. 将本文档状态改为 Accepted Governance Baseline；
2. 将 Current 文档更新为 Awaiting Startup Authorization；
3. 由 Owner 决定合并或使治理基线进入 `main`；
4. 仍不创建 `feat/v1-autonomous-build`；
5. 仍不开始 S1–S10。

Governance Finalization 结束后进入 State 2，而不是 State 3。

## Program Startup

仅在 Governance Baseline 已进入 `main` 且 Owner 给出独立、明确的 startup instruction 后执行：
...
# Program Coverage Upon Explicit Startup Authorization

只有在 State 3 — Program Authorized / Running 中，一次性执行授权才覆盖以下 Macro Stages：
...
* 本计划或当前工作与 `DEVELOPMENT_CONTRACT.md` 存在实质冲突；
* 完成当前工作必须修改、绕过或降低 `DEVELOPMENT_CONTRACT.md` 的要求；
* `Docs/CURRENT_STATE.md`、`Docs/CURRENT_TASK.md` 与适用执行计划之间存在会改变是否可以启动或继续执行的冲突；
* V1 Scope 无法支持某项必需实现；
...
当前没有持久化 Development Team 或真机签名条件本身不触发中途升级。Codex 应完成 Autonomous Candidate Technical Gate，并将真机检查保留给最终 Owner Manual Validation。只有已有具体证据表明缺少真机结果会使继续执行危及用户数据完整性、平台能力判断或造成不可逆设计时，才按照升级条件暂停。
...
这是 Codex 在 S1–S10 自主执行期间必须完成、并用于决定是否可以进入下一已授权 Stage 的技术门槛。按变更范围适用时，包括：

* 在可用 iPhone Simulator 上构建和启动；
* Unit Tests；
* UI Tests；
* in-memory 或 temporary on-disk SwiftData；
* temporary media directory；
* generated / synthetic fixtures；
* 重启 App、重新打开测试数据库和重新读取媒体；
* 隔离环境中的 Export / Import；
* 失败注入；
* 数据、关系和媒体一致性测试；
* 模拟器中的关键用户路径；
* 静态检查；
...
当适用的 Autonomous Candidate Technical Gate 通过后，Codex 可以自动进入下一已授权 Stage。该技术门槛不要求真实 iPhone、Owner 真实数据、Development Team 或真机签名。

## Owner-deferred Physical-device Validation

以下检查统一推迟到最终 Owner Manual Validation，不阻塞 S3 → S4、S9 → S10 或 V1 Candidate Technical Completion：

* 在真实 iPhone 上安装和启动；
* 真实 Photos Picker 和相册权限行为；
* 真实设备上的图片选择体验；
* 真实设备上的性能、内存、磁盘和交互体验；
* 使用 Owner 真实生活内容进行验证；
...
Codex 不得声称这些检查已经完成。最终交付必须生成 `Owner Manual Validation Checklist`，清楚列出待 Owner 完成的真机和真实生活检查。这些检查是 Owner 接受 V1 Candidate、开始正式 Dogfooding 或进入 30 天 Exit Observation 前的门槛，但不是自主工程 Stage 之间的继续条件。

如果某个具体问题已有证据表明无法在没有真机结果的情况下安全继续，并且涉及用户数据完整性、Apple 平台能力或不可逆设计，Codex 仍必须按照 Mandatory Escalation Conditions 暂停。
...
* `Docs/CURRENT_TASK.md` 在 State 1/2 应保持 Governance Review 或 Awaiting Startup Authorization 边界，在 State 3 应保持 V1 Autonomous Build Program 边界；
...
* 默认治理模式仍是逐子 Stage 审批；
* Governance 接受不等于 Program 启动；
* 只有 Owner 明确发出 startup instruction 且 `Docs/CURRENT_TASK.md` 已记录启动授权后，V1 才采用一次性 Autonomous Build 授权；
* State 3 中的 Program coverage 为 Macro Stage S1–S10；
* S1–S10 保留为工程和验证边界；
* 在 State 3 中不再逐 Stage 等待 Owner 批准；
* Codex 在 State 3 的批准范围内自主规划、实现、测试、修复和提交；
* 普通、可逆、可验证的实现决策不升级；
...
Program 进入 State 3 后，具体 UI、局部类型设计、文件组织和普通实现选择由 Codex 在本计划范围内自主决定。
